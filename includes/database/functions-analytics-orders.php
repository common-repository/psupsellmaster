<?php
/**
 * Functions - Database - Analytics - Orders.
 *
 ...
 */
function psupsellmaster_db_analytics_orders_get_defaults() {
	// Set the defaults.
	$defaults = array(
		'order_product_id'   => 0,
 ...
 */
function psupsellmaster_db_analytics_orders_get_columns() {
	// Set the columns.
	$columns = array(
		'id',
		'order_product_id',
 ...
 */
function psupsellmaster_db_analytics_orders_count() {
	// Set the SQL select.
 ...
 */
function psupsellmaster_db_analytics_orders_select( $where = array(), $order_by = array() ) {
	// Get the table columns.
	$table_columns = psupsellmaster_db_analytics_orders_get_columns();
	...
	if ( ! empty( $where ) ) {
		...
		// Check if the order product id is not empty.
		if ( isset( $where['order_product_id'] ) ) {
			// Add conditions to the SQL where.
			array_push( $sql_where, PsUpsellMaster_Database::prepare( 'AND `od`.`order_product_id` = %d', $where['order_product_id'] ) );
		}

		// Check if the related product id is not empty.
		if ( isset( $where['related_product_id'] ) ) {
			// Add conditions to the SQL where.
			array_push( $sql_where, PsUpsellMaster_Database::prepare( 'AND `od`.`related_product_id` = %d', $where['related_product_id'] ) );
		}
 ...
 */
function psupsellmaster_db_analytics_orders_insert( $data ) {
	// Set the table name.
	$table_name = PsUpsellMaster_Database::get_table_name( 'psupsellmaster_analytics_orders' );

	// Get the table defaults.
	$table_defaults = psupsellmaster_db_analytics_orders_get_defaults();
 ...
 */
function psupsellmaster_db_analytics_orders_update( $data, $where ) {
	// Set the table name.
	$table_name = PsUpsellMaster_Database::get_table_name( 'psupsellmaster_analytics_orders' );
 ...
 */
function psupsellmaster_db_analytics_orders_delete( $where ) {
	// Set the table name.
	$table_name = PsUpsellMaster_Database::get_table_name( 'psupsellmaster_analytics_orders' );
 ...
 */
function psupsellmaster_db_analytics_orders_truncate() {
	// Truncate the database table.
	return psupsellmaster_db_truncate_table( 'psupsellmaster_analytics_orders' );
}
 ...
 */
function psupsellmaster_db_get_analytics_order_by( $column, $value ) {
	// Set the analytics order.
	$analytics_order = false;

	// Get the analytics orders.
	$analytics_orders = psupsellmaster_db_analytics_orders_select( array( $column => $value ) );

	// Check if the analytics orders is an array.
	if ( is_array( $analytics_orders ) ) {
		// Set the analytics order.
		$analytics_order = array_pop( $analytics_orders );
	}

	// Return the analytics order.
